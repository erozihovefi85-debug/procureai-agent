...
import { Message, UploadedFile, LoadingState, DifyGeneratedFile } from '../types';
import { 
...
const ChatArea: React.FC<ChatAreaProps> = ({ 
    messages, isLoading, onSend, onCancel, placeholder, emptyState, readOnly, currentNodeName
}) => {
          ...
          return (
            <div key={msg.id} className={`flex items-start gap-3 max-w-4xl mx-auto group relative transition-all duration-300 ${isSelectionMode ? 'cursor-pointer' : ''} ${msg.role === 'user' ? 'flex-row-reverse' : ''}`} onClick={() => isSelectionMode && toggleMessageSelection(msg.id)}>
              ...
              <div className={`flex flex-col max-w-[85%] md:max-w-[85%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                {showBubble && (
                  <div className={`relative px-4 py-3 rounded-2xl shadow-sm leading-relaxed pb-8 min-w-[40px] w-full ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-white border border-slate-100 text-slate-700 rounded-tl-sm'}`}>
